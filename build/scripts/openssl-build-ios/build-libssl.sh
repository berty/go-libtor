...
echo_help()
{
  ...
  echo "     --cleanup                     Clean up build directories (bin, include, lib, src) before starting build"
...
if [ "${CLEANUP}" == "true" ]; then
  ...
  if [ -d "${CURRENTPATH}/include" ]; then
    rm -r "${CURRENTPATH}/include"
  fi
...
if [ ${#OPENSSLCONF_ALL[@]} -gt 1 ]; then
  ...
  cp "${CURRENTPATH}/template/opensslconf-template.h" "${OPENSSLCONF_INTERMEDIATE}"
